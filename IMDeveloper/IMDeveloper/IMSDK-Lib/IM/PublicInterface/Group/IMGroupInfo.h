...
#pragma mark - 修改群推送通知权限的回调方法

/**
 @method 
 @brief 修改群推送通知权限成功的回调方法
 @param receivePushMessage    是否接收群推送通知
 @param timeIntervalSince1970 1970年到客户端修改时间的秒数
 */
- (void)didSetReceivePushMessage:(BOOL)receivePushMessage clientSendTime:(UInt32)timeIntervalSince1970;


/**
 @method
 @brief 修改群推送通知权限失败的回调方法
 @param receivePushMessage    是否接收群推送通知
 @param timeIntervalSince1970 1970年到客户端修改时间的秒数
 @param error                 修改失败的错误信息
 */
- (void)failedToSetReceivePushMessage:(BOOL)receivePushMessage
...
/**
 ...
 @brief 是否接收群推送消息
 */
@property (nonatomic, readonly) BOOL receivePushMessage;
...
/**
 @method
 @brief 修改接收群组消息权限（异步方法，从服务器获取）
 @param success               修改接收群组消息权限成功的block回调
 @param groupInfo             修改接收群组消息权限成功的block回调的返回参数，IMGroupInfo类型,该参数为提交成功后的群对象。
 @param failure               修改接收群组消息权限失败的block回调
 @param error                 修改接收群组消息权限的错误信息
 @return 1970年到客户端修改时间的秒数
 */
- (UInt32)setReceivePushMessage:(BOOL)receivePushMessage success:(void (^)(void))success failure:(void (^)(NSString *error))failure;
@end
